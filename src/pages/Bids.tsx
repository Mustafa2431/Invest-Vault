...
import { TrendingUp, CheckCircle, XCircle, Clock, Send, MessageSquare, Mail, Loader } from 'lucide-react';
  ...
  const loadBids = async () => {
    ...
    if (profile.role === 'investor') {
      ...
      if (startupData) {
        const { data } = await supabase
          .from('bids')
          .select(`*, startup:startups(id, company_name, logo_url), investor:profiles(id, full_name, email)`)
...
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbz3iDKWYuBMUqtRYyqKdmeVauuQF8fxWJxZ3QCAxs-qoN-91fmSoL_GTLoSxPzJ71ea/exec';
...
— Invest Vault Team`;
  ...
  const EmailBadge = ({ bidId }: { bidId: string }) => {
    ...
    const cfg = {
      sending: { cls: 'bg-blue-600/20 border-blue-500/30 text-blue-400',   icon: <Loader size={12} className="animate-spin" />, label: 'Sending email…'  },
      sent:    { cls: 'bg-green-600/20 border-green-500/30 text-green-400', icon: <Mail size={12} />,                            label: 'Email sent!'      },
      error:   { cls: 'bg-red-600/20 border-red-500/30 text-red-400',       icon: <XCircle size={12} />,                         label: 'Email failed'     },
    }[status];

    return (
      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border mt-2 ${cfg.cls}`}>
        {cfg.icon}{cfg.label}
      </span>
    );
  };
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center space-x-4 mb-4">
                        {bid.startup.logo_url ? (
                          <img src={bid.startup.logo_url} alt={bid.startup.company_name} className="w-12 h-12 rounded-lg flex-shrink-0" />
                        ) : (
                      ...
                      {profile?.role === 'investor' && (bid.status === 'accepted' || bid.status === 'rejected') && (
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border mt-2
                          ${bid.status === 'accepted'
                            ? 'bg-green-600/10 border-green-600/20 text-green-400'
                            : 'bg-red-600/10 border-red-600/20 text-red-400'}`}>
                          <Mail size={12} />
                          {bid.status === 'accepted' ? 'Acceptance email was sent to you' : 'Rejection email was sent to you'}
                        </span>
                      )}
                    </div>
                    ...
                    {profile?.role === 'startup' && bid.status === 'pending' && (
                      <div className="flex flex-col space-y-2 flex-shrink-0">
                        ...
                        >
                          {isBidLoading ? <Loader size={14} className="animate-spin" /> : <CheckCircle size={14} />}
                          <span>{isBidLoading ? 'Accepting…' : 'Accept'}</span>
                        </Button>
            ...
            <Card className="w-full max-w-md shadow-2xl">
              <div className="p-6">
                ...
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white">Reject Bid</h3>
                    <p className="text-sm text-slate-400">{bid.startup.company_name}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2 p-3 bg-blue-600/10 border border-blue-500/20 rounded-lg mb-5">
                  ...
                  <p className="text-sm text-blue-300 leading-snug">
                    ...
                    <span className="font-semibold text-white">{bid.investor?.email}</span>{' '}
                ...
                <div className="mb-5">
                  ...
                  <textarea
                    ...
                    rows={4}
                  />
                  <p className="text-xs text-slate-500 mt-1 text-right">{rejectMessage.length}/500</p>
                </div>
                ...
                <div className="flex gap-3">
                  <Button
                    variant="outline"
                    onClick={() => { setRejectingBidId(null); setRejectMessage(''); }}
                    className="flex-1"
                    disabled={isSending}
                  >
                    Cancel
                  ...
                  >
                    {isSending
                      ? <><Loader size={15} className="animate-spin" /><span>Sending…</span></>
                      : <><Send size={15} /><span>Reject &amp; Send Email</span></>}
                  </Button>
                </div>
              </div>
            </Card>
