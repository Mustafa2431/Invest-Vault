  ...
  const loadBids = async () => {
    ...
    if (profile.role === "investor") {
      const { data } = await supabase
        .from("bids")
        .select(`*, startup:startups(id, company_name, logo_url, current_funding)`)
      ...
      if (startupData) {
        const { data } = await supabase
          .from("bids")
          .select(
            `*, startup:startups(id, company_name, logo_url, current_funding), investor:profiles(id, full_name, email)`,
          )
...
— Invest Vault Team`;
  ...
  const handleAcceptBid = async (bid: BidWithStartup) => {
    ...
    if (!bidError) {
      // 2. Update current_funding on the startup by summing all accepted bids
      //    (re-calculate from DB to avoid race conditions)
