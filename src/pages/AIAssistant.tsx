...
import { Send, Brain, Sparkles, TrendingUp, Shield, DollarSign, PieChart } from 'lucide-react';
...
function MessageBubble({ message, isUser }: { message: Message; isUser: boolean }) {
  const formattedContent = message.content
    ...
    .replace(/\*(.*?)\*/g, '<em>$1</em>')
    .replace(/`(.*?)`/g, '<code class="bg-slate-700/60 px-1 rounded text-blue-300 text-sm font-mono">$1</code>')
    .replace(/\n/g, '<br/>');
...
export function AIAssistant() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: "Hello! I'm your AI investment assistant powered by Gemini. I can help you understand investment terms, analyze startup metrics, guide you through the bidding process, and much more.\n\nWhat would you like to know today?",
      timestamp: new Date(),
    },
  ]);
      ...
      <div className="p-6 h-screen flex flex-col">
        ...
        <div className="flex items-center justify-between mb-4 flex-shrink-0">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-0.5">Expert guidance on startup investing</p>
          </div>
        </div>
        ...
        <div className="flex-1 bg-slate-800/30 border border-slate-700/50 rounded-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            {showQuickPrompts && !loading && (
              <div className="pt-4">
                <p className="text-xs text-slate-500 mb-3 text-center">Try asking about:</p>
          ...
          <div className="p-4 border-t border-slate-700/50 bg-slate-900/30">
            <div className="flex items-end gap-3">
              ...
              >
                <Send size={18} className={loading ? 'text-slate-500' : 'text-white'} />
              </button>
            </div>
            <p className="text-xs text-slate-600 mt-2 text-center">
              AI responses are for informational purposes only. Consult a financial advisor for personalized advice.
            </p>
          </div>
        </div>
      </div>
